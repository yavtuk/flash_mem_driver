 ...
 *
 */
uint32_t fmem_determine_erase_volume(struct __fmem_layer * const fml, const __fmem_layer_data * const wdata)
{
    uint32_t count_addr;
    uint32_t count_data;
    uint32_t erase_volume;
    uint32_t wr_len;
    ...
    while (count_data < wdata->len) {
        /* determine a data block to write */
        wr_len = FML_GET_WRITE_DATALEN(fml, count_addr);
        wr_len = ((wdata->len - count_data) > wr_len)
                              ? wr_len : (wdata->len - count_data);


        /* check at new sector */
        if (FML_IS_NEW_SECTOR(fml, count_addr)) {
            erase_volume += fml->descriptor->fmh->descriptor->SECTOR_SIZE;
        }
